...
void set_sock_linger(int sock)
{
    ...
    socklen_t lingeroptlen = sizeof(lingeropt);

    memset(&lingeropt, 0, sizeof(lingeropt));
    lingeropt.l_onoff = 1;
    lingeropt.l_linger = 0;

    if (setsockopt(sock, SOL_SOCKET, SO_LINGER, &lingeropt, lingeroptlen) < 0) {
